...
def sample_noise(batch_size, dim, seed=None):
    ...
    return 2*torch.rand(batch_size, dim).type(dtype)-1
...
def get_optimizer(model):
    ...
    optimizer=torch.optim.Adam(model.parameters(), lr=1e-3, betas=(0.5,0.999), eps=1e-08)
...
def build_dc_classifier():
    ...
    model=nn.Sequential(
      ...
      nn.MaxPool2d(2,stride=2),
      nn.Flatten(start_dim=1),
      nn.Linear(1024,out_features=1024,bias=True),
      nn.LeakyReLU(0.01),
