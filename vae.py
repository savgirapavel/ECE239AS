...
class CVAE(nn.Module):
    ...
    def forward(self, x, labels):
        ...
        labels_onehot = labels.reshape(labels.size(0), -1)
        encoder_input=torch.cat([x,labels_onehot],dim=1)
        hidden_features = self.encoder(encoder_input)
        mu = self.mu_layer(hidden_features)
        logvar = self.logvar_layer(hidden_features)
        z = reparametrize(mu, logvar)
...
def reparametrize(mu, logvar):
    ...
    std = torch.pow(torch.exp(logvar),0.5)
...
def loss_function(x_hat, x, mu, logvar):
    ...
    kl_divergence=(-1/2)*(1+logvar-torch.pow(mu,2)-torch.exp(logvar))
    kl = kl_divergence.sum()
    loss= (bce_loss+kl)/x.size(0)
